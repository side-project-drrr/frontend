<p align="center">
    <img src="/src/assets/logo.png">
</p>
## ABOUT

<div align='center'>
    <p>https://drrr.kr/</p>
    <h3>다양한 회사의 기술 블로그를 카테고리별로 분류하여, <br/>
    사용자가 원하는 주제의 정보를 쉽게 찾을 수 있는 시스템을 개발 </h3>
    <br />    
</div>

## TEAM

<div align='center'>
<table> 
  <tbody>
    <tr>            
       <td align="center"><a href="https://github.com/rondido"><img src="https://avatars.githubusercontent.com/u/55516901?v=4" width="100px;" alt=""/><br /><sub><b>박진현</b></sub></a><br /></td>
       <td align="center"><a href="https://github.com/rondido"><img src="https://avatars.githubusercontent.com/u/70056958?v=4" width="100px;" alt=""/><br /><sub><b>박진영</b></sub></a><br /></td>
    </tr>
  </tbody>
</table>
</div>

<br />

## ✨ 프론트엔드 기술 스택

<img src="/src/assets/frontStack.png">

## 인프라 기술 스택

<img src="/src/assets/infraStack.png">

## 프론트엔드 인프라 구조

<img src="/src/assets/allStack.png">

## 폴더 구조

```
   📦github/workflows
   📦husky
   📦storybook
   📦vscode
   📦cypress
   📦nginx
   📦public
   📦security
   📦src
    ┣ 📂apis
    ┣ 📂assets
    ┣ 📂components
    ┣ 📂constants
    ┣ 📂context
    ┣ 📂hooks
    ┣ 📂mocks => msw를 관련 폴더
    ┣ 📂pages
    ┣ 📂recoil
    ┣ 📂repository
    ┣ 📂service
    ┣ 📂style
    ┣ 📂stories
    ┣ 📂ThemeContext
    ┣ 📂webpush
    ┣ App.tsx
    ┣ Main.tsx

```

## 실행 방법

```bash
yarn version 3.6.3

yarn install

yarn run dev

```

## Work-flow

https://whimsical.com/controller-TtEKRiFpR2HX8KxJKky9RR

```
password:1234
```

<br/>

## 🤝 팀문화

1. 매주 미팅을 가져요

    - 매주 월요일 매주 이슈나 기타 사항을 공유해요.
    - 해결하지 못한 이슈가 있다면 함께 트러블 슈팅을 통해 해결해요.

2. 매주 함께 회고해요.

    - 매주 수요일 저녁에 주간 회고를 가져요.
    - 한 주 동안 세웠던 목표와 결과를 공유합니다.
    - 이에 대한 내용들을 노션에 기록하고 공유해요.

3. 각자의 의견을 존중하고, 조율해요.

    - 반대 의견도 자유롭게 내요
    - 정해진 의견은 그 때부터 팀의 의견이니 문제가 생겨도 팀원을 탓하지 않아요.

4. 이런 문화도 존재해요.
    - PR은 리뷰어가 코드 리뷰 후 수정한 후에 리뷰어가 머지할 수 있어요.
